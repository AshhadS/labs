...
$requestUri = preg_replace('/[\r\n]/', '', $_SERVER["REQUEST_URI"] ?? "/gold-rates/");
$productionHost = "ashhadslabs.com";
$requestHost = strtolower(preg_replace('/:\d+$/', '', $host));
$isProduction = in_array($requestHost, [$productionHost, "www." . $productionHost], true);

if ($isProduction && ($isHttps || $requestHost !== $productionHost)) {
    header("Location: http://ashhadslabs.com" . $requestUri, true, 301);
    exit;
}

$scheme = $isHttps ? "https" : "http";
$requestPath = strtok($_SERVER["REQUEST_URI"] ?? "/gold-rates/", "?");

$canonicalUrl = $isProduction
    ? "http://" . $productionHost . "/gold-rates/"
